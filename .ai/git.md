...
- コミット開始の条件は、ユーザー最新指示に `/commit` 系コマンドが明示されていること（ただしPR作成またはPR更新時の未コミット変更に対する自動実行は例外）
- `/commit` / `/c` / `/commit!` / `/c!` の実行中に発生した確認への返答は、同一手順内の継続指示として扱う
...
- PR作成またはPR更新の実行時に未コミット変更がある場合は、`/commit!` / `/c!` 相当（`git add -A`・ステージ対象確認・必要時の継続確認・先頭候補で即コミット）を自動実行してから進める

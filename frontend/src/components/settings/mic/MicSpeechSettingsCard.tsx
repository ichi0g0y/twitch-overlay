        ...
        <div className="space-y-4 pt-2 border-t border-gray-800/60">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">認識テキストをWebSocketで送信</p>
            </div>
            <Switch
          ...
          {(overlaySettings?.mic_transcript_bouyomi_enabled ?? false) && (
            <div className="space-y-2">
              <Label htmlFor="mic-bouyomi-url">接続先URL</Label>
              <Input
                id="mic-bouyomi-url"
                placeholder="ws://localhost:50002/ws/"
                value={overlaySettings?.mic_transcript_bouyomi_url ?? ''}
                onChange={(e) => updateOverlaySettings({ mic_transcript_bouyomi_url: e.target.value })}
              />
              <p className="text-xs text-gray-500 dark:text-gray-400">空欄の場合は ws://localhost:50002/ws/ を使用</p>
            </div>
          )}
        </div>

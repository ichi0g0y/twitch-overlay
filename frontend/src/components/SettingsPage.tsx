import { Bluetooth, Bug, Eye, EyeOff, FileText, Layers, Monitor, Moon, Music, Pause, Play, Radio, RefreshCw, Settings2, SkipBack, SkipForward, Square, Sun, Upload, Volume2, Wifi, X } from 'lucide-react';
...
import { GetAuthURL, GetAllSettings, UpdateSettings, GetFeatureStatus, GetPrinterStatus, GetServerPort, GetServerStatus, RestartWebServer, GenerateFontPreview, UploadFont, DeleteFont, ScanBluetoothDevices, ReconnectPrinter, TestPrint, GetMusicPlaylists } from '../../wailsjs/go/main/App';
...
export const SettingsPage: React.FC = () => {
  ...
  const [serverStatus, setServerStatus] = useState<{
    oauth_server: { port: number; running: boolean };
    web_server: { port: number; running: boolean };
  } | null>(null);
  ...
  useEffect(() => {
    ...
    // プリンター接続イベントをリッスン
    const unsubscribePrinter = EventsOn('printer_connected', (connected: boolean) => {
      // プリンター接続状態が変更されたら、設定とステータスを再取得
      fetchAllSettings();
      fetchPrinterStatus();
    });
  ...
  useEffect(() => {
    ...
    GetServerPort()
      ...
      .catch(e => {
        ...
        setWebServerPort(30303); // デフォルト値
      });
  }, []);
  
  // WebSocket接続を統合（音楽状態とストリーム状態の監視）
  useEffect(() => {
    let unsubMusicStatus: (() => void) | null = null;
    let unsubStreamStatus: (() => void) | null = null;
    ...
    const loadMusicState = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('Loaded music state from server:', serverState);
          if (serverState) {
            setMusicStatus(prev => ({
              ...prev,
              ...serverState,
              playlist_name: serverState.playlist_name || undefined
            }));
          }
        }
      } catch (error) {
    ...
    // WebSocket接続を一度だけ初期化
    initWebSocket().then(wsClient => {
      // 音楽状態更新メッセージを処理
      unsubMusicStatus = wsClient.on('music_status', (status) => {
        // デバッグ用: プログレス情報を詳細ログ
        console.debug('Music status updated:', {
          current_time: status.current_time,
          duration: status.duration,
          progress: status.progress,
          is_playing: status.is_playing
        });
        // 完全に新しいオブジェクトとして設定（強制的な再レンダリング）
        setMusicStatus(prev => ({
          ...prev,
          ...status,
          // 値が確実に更新されるように明示的に設定
      ...
      unsubStreamStatus = wsClient.on('stream_status_changed', (data) => {
  ...
  const sendMusicControlCommand = async (endpoint: string, body?: any) => {
    // デバウンス処理: 既に処理中なら何もしない
    if (isControlDisabled) {
      console.log('Music control command ignored (debouncing)');
      return;
    }
    ...
    try {
      ...
      // webServerPortが設定されていればそれを使用、なければ動的に取得
      let port = webServerPort;
      ...
      if (!port) {
        try {
          ...
          // GetServerPortが失敗した場合は8080を使用
          port = 8080;
          console.log('Using default port 8080 for music control');
        }
      }
      
      const url = `http://localhost:${port}/api/music/control/${endpoint}`;
  ...
  // シークバーの値を強制的に更新（Reactの制御コンポーネント問題の回避）
  useEffect(() => {
    if (seekBarRef.current && musicStatus.current_time !== undefined) {
      // DOM要素を直接更新
      seekBarRef.current.value = String(musicStatus.current_time);
      
      // スタイルも更新（プログレスバーの視覚的更新）
      const progress = (musicStatus.current_time / (musicStatus.duration || 1)) * 100;
      seekBarRef.current.style.background = `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${progress}%, #e5e7eb ${progress}%, #e5e7eb 100%)`;
      
      console.debug(`[SeekBar Force Update] time: ${musicStatus.current_time}, progress: ${progress.toFixed(1)}%`);
    }
  }, [musicStatus]); // musicStatus全体を監視
  ...
  const handleTwitchAuth = async () => {
    try {
      // WailsのAPIでOAuth URLを取得
      const authUrl = await GetAuthURL();
      
      // 外部ブラウザで開く
      await BrowserOpenURL(authUrl);
  ...
  const handlePrinterReconnect = async () => {
    ...
    try {
      ...
      const errorMessage = err.message || err.toString() || '接続に失敗しました';
      toast.error(`再接続エラー: ${errorMessage}`);
    } finally {
  ...
  const handleTestPrint = async () => {
    ...
    try {
      // Wailsのテストプリント関数を呼び出す
      await TestPrint();
      toast.success('テストプリントを送信しました');
    } catch (err: any) {
      const errorMessage = err.message || err.toString() || 'テストプリントに失敗しました';
      toast.error(`テストプリントエラー: ${errorMessage}`);
    } finally {
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={async () => {
                    ...
                    console.log('Opening overlay URL:', url);
                    BrowserOpenURL(url);
                  }}
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    console.log('Opening overlay URL (Debug):', url);
                    BrowserOpenURL(url);
                  }}
                  className="flex items-center space-x-1"
                >
                  <Monitor className="w-3 h-3" />
                  <span>オーバーレイ表示(Debug)</span>
                </Button>
              </div>
            </div>
            <Button
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <div className={`w-3 h-3 rounded-full ${
                      serverStatus?.oauth_server?.running && serverStatus?.web_server?.running ? 'bg-green-500' :
                      serverStatus?.oauth_server?.running || serverStatus?.web_server?.running ? 'bg-yellow-500' : 'bg-red-500'
                    }`} />
                    <span className="font-medium dark:text-gray-200">内部サーバー</span>
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                      {serverStatus?.oauth_server?.running && serverStatus?.web_server?.running ? '正常' :
                       serverStatus?.oauth_server?.running || serverStatus?.web_server?.running ? '一部停止' : '停止中'}
                    </span>
                  </div>
                  {serverStatus && (
                    <div className="ml-5 text-sm space-y-1">
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${serverStatus.oauth_server.running ? 'bg-green-500' : 'bg-red-500'}`} />
                        <span className="text-gray-600 dark:text-gray-300">
                          OAuth ({serverStatus.oauth_server.port}): {serverStatus.oauth_server.running ? '起動中' : '停止中'}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${serverStatus.web_server.running ? 'bg-green-500' : 'bg-red-500'}`} />
                        <span className="text-gray-600 dark:text-gray-300">
                          Web ({serverStatus.web_server.port}): {serverStatus.web_server.running ? '起動中' : '停止中'}
                        </span>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="w-12 h-12 flex-shrink-0">
                          {musicStatus.current_track.has_artwork ? (
                            <img
                              src={`http://localhost:${webServerPort}/api/music/track/${musicStatus.current_track.id}/artwork`}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={async (value) => {
                        ...
                        // プレイリスト選択を永続化
                        try {
                          const { buildApiUrlAsync } = await import('../utils/api');
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">音楽停止</p>
                    </div>
                    
                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      <div className="flex items-center space-x-2 mb-2">
                        <span className="px-2 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 text-xs font-mono rounded">POST</span>
                        <span className="font-mono text-sm dark:text-gray-300">/api/music/control/toggle</span>
                      </div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">再生/一時停止切り替え</p>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">シーク位置変更 (body: {`{"position": seconds}`})</p>
                    </div>
